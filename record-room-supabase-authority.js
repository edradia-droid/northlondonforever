...
function lineupRows(f,s){
  const rows=[];const add=(team,lineup,subs)=>{const outMap=new Map((subs||[]).map(x=>[playerKey(canonicalPlayerName(team,x.out)),x]));(lineup||[]).filter(Boolean).forEach((name,i)=>{const player_name=canonicalPlayerName(team,name);const sub=outMap.get(playerKey(player_name));rows.push({team_name:team,player_name,is_starter:true,minute_on:0,minute_off:sub&&Number.isFinite(Number(sub.outMin))?Number(sub.outMin):null,pitch_slot:String(i+1)});});(subs||[]).filter(x=>x.in).forEach(x=>rows.push({team_name:team,player_name:canonicalPlayerName(team,x.in),is_starter:false,minute_on:Number(x.inMin??x.outMin)||0,minute_off:null}));};add(f.home,s.homeLineup,s.homeSubs);add(f.away,s.awayLineup,s.awaySubs);return rows;
}
function substitutionRows(f,s){const rows=[];const add=(team,subs)=>(subs||[]).forEach(x=>{if(x.out&&x.in)rows.push({team_name:team,player_out:canonicalPlayerName(team,x.out),player_in:canonicalPlayerName(team,x.in),minute:Number(x.inMin??x.outMin)||0});});add(f.home,s.homeSubs);add(f.away,s.awaySubs);return rows;}
...
async function hydrateCore(){
  ...
  const marker=document.getElementById('buildMarker');if(marker)marker.textContent=`BUILD V51 • SUPABASE LIVE • ${imported} MATCHES HYDRATED • CANONICAL PLAYERS • MOBILE + PC`;
...
function hydrate(){
  ...
  const marker=document.getElementById('buildMarker');if(marker)marker.textContent='BUILD V51 • SUPABASE CONNECTING…';
  hydrateFlight=hydrateCore().catch(error=>{
    const m=document.getElementById('buildMarker');if(m)m.textContent='BUILD V51 • SUPABASE HYDRATION ERROR';
    throw error;
...
function start(){document.addEventListener('click',e=>{const b=e.target.closest?.('button');if(!b)return;if(b.classList.contains('detail-save')||/SAVE MATCH DETAILS/i.test(b.textContent||''))setTimeout(()=>saveOpenFixture(),80);},true);document.getElementById('savePlayer')?.addEventListener('click',()=>setTimeout(()=>savePlayers().catch(console.error),80));}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();
window.NL4RecordRoomSupabaseAuthority={hydrate,saveFixture,savePlayers,saveOpenFixture,canonicalPlayerName,version:'20260909-v51'};
})();