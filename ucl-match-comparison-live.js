...
function html(home,away,h,a,locked){
  ...
  const liveHp=probability(h,a),hp=locked?.home_win_probability!=null?Number(locked.home_win_probability):liveHp,ap=locked?.away_win_probability!=null?Number(locked.away_win_probability):100-hp,hc=colours[clean(home)]||['#3454a5','#16264f'],ac=colours[clean(away)]||['#b18b32','#3a2c12'];
  const lockedText=locked?.probability_locked_at?'Pre-match probability locked at kickoff.':'Calculated from the current synced UCL table until kickoff.';
  return `<div class="ucl-comparison" data-live-ucl-comparison="1"><div class="comparison-table-wrap"><table class="comparison-table"><thead><tr><th>METRIC</th><th>${esc(clean(home))}</th><th>${esc(clean(away))}</th></tr></thead><tbody>${metrics.map(m=>`<tr><th>${esc(m[0])}</th><td>${esc(get(h,m[1]))}</td><td>${esc(get(a,m[1]))}</td></tr>`).join('')}</tbody></table></div><div class="probability-card" style="--home-primary:${hc[0]};--home-secondary:${hc[1]};--away-primary:${ac[0]};--away-secondary:${ac[1]}"><div class="probability-heading"><strong>Win Probability</strong><small>${esc(lockedText)}</small></div><div class="probability-names"><span>${esc(clean(home))}</span><span>${esc(clean(away))}</span></div><div class="probability-track" role="img" aria-label="${esc(clean(home))} ${hp} percent, ${esc(clean(away))} ${ap} percent"><div class="probability-side probability-home" style="width:${hp}%"><strong>${hp}%</strong></div><div class="probability-side probability-away" style="width:${ap}%"><strong>${ap}%</strong></div></div><p class="probability-note">${locked?.probability_locked_at?'Locked pre-match estimate — it will not change after kickoff.':'Updates automatically before kickoff only.'}</p></div></div>`;
}
