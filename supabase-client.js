...
if (NL4_IS_RECORD_ROOM) {
  ...
  const playerFromSeed=([name,position,number])=>({
  ...
  function registerArsenal(){
    try{
      ...
      const merged=arsenalBase().map(p=>({
        ...p,
        ...(savedByName.get(norm(p.name))||{}),
        name:p.name,position:p.position,number:p.number
      }));
      (existing.players||[]).forEach(p=>{
        if(p?.name && !merged.some(x=>norm(x.name)===norm(p.name))) merged.push({...playerFromSeed([p.name,p.position||'Midfielder',p.number??null]),...p});
      });
      db[ARSENAL]={...existing,club:{...blankClub(),...(existing.club||{})},players:merged,fixtureData:existing.fixtureData||{}};

      // V9 built PLAYER_DIRECTORY before Arsenal existed. Replace the native roster
      // accessor so every later render, player dropdown, lineup and event selector
      // receives Arsenal players instead of resetting them to an empty array.
      const originalRosterForTeam=typeof window.rosterForTeam==='function'?window.rosterForTeam:null;
      window.rosterForTeam=function(team){
        if(team===ARSENAL){
          const current=(db[ARSENAL]?.players||[]);
          return current.length?current.map(p=>({...p})):arsenalBase();
        }
        return originalRosterForTeam?originalRosterForTeam(team):[];
      };

      // V9 explicitly returned [] for Arsenal here. Override it so lineup,
      // substitution, goal, assist, card and MOTM controls all receive the roster.
      const originalTeamPlayers=typeof window.teamPlayers==='function'?window.teamPlayers:null;
      window.teamPlayers=function(team){
        if(team===ARSENAL) return db[ARSENAL]?.players||[];
        return originalTeamPlayers?originalTeamPlayers(team):(db[team]?.players||[]);
      };

      // Reuse canonical match records already saved on Arsenal's opponents.
      if(typeof ALL_FIXTURES!=='undefined'){
        ALL_FIXTURES.filter(f=>f.home===ARSENAL||f.away===ARSENAL).forEach(f=>{
          if(db[ARSENAL].fixtureData?.[f.id]) return;
          const opponent=f.home===ARSENAL?f.away:f.home;
          const copy=db[opponent]?.fixtureData?.[f.id];
          if(copy){
            db[ARSENAL].fixtureData=db[ARSENAL].fixtureData||{};
            db[ARSENAL].fixtureData[f.id]=clone(copy);
          }
        });
      }
      ...
      // Make the corrected 20-team build immediately obvious after load.
      const teamSelect=document.getElementById('teamSelect');
      const inputTeam=document.getElementById('inputTeam');
      if(teamSelect) teamSelect.value=ARSENAL;
      if(inputTeam) inputTeam.value=ARSENAL;
      ...
      if(marker) marker.textContent='BUILD V13 • ALL 20 PREMIER LEAGUE TEAMS • ARSENAL NATIVE';
      ...
      const manualTitle=document.querySelector('.admin h3');
      if(manualTitle && /Other 19 Teams/i.test(manualTitle.textContent||'')) manualTitle.textContent='All 20 Teams Stat Input';

      if(typeof render==='function') render();
    }catch(err){
      console.warn('[NL4 Record Room] Arsenal registration failed:',err);
    }
  }

  // record-room.html loads this file after its native V9 engine, so all V9 globals
  // are available synchronously here. No polling or MutationObserver is needed.
  registerArsenal();
  ...
  const addButton=()=>{
    ...
    b.id='rrLoadMaintenance';b.type='button';b.textContent='Load full data tools';
    ...
    b.addEventListener('click',async()=>{
      ...
      try{await window.NL4LoadRecordRoomMaintenance();b.dataset.loaded='1';b.textContent='Data tools loaded';}
      catch(error){console.warn('[NL4 Record Room] Manual data-tool load failed:',error);b.disabled=false;b.textContent='Retry data tools';}
    });
