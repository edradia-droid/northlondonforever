...
function ensurePanel(){
  let p=document.getElementById('nl4AdminTop4Interpretation');
  if(p) return p;
  const table=document.getElementById('titleProbabilityTable');
  if(!table) return null;
  p=document.createElement('section'); p.id='nl4AdminTop4Interpretation';
  p.innerHTML=`<div class="head"><div><span class="label">ADMIN MODEL INTERPRETATION • TITLE RACE TOP FOUR</span><h3>How Recent Matches Changed Their Title Hopes</h3></div><strong class="status" id="nl4AdminTop4Status">READING ADMIN MODEL…</strong></div><p class="summary" id="nl4AdminTop4Summary">Waiting for the Admin Model probability table…</p><div class="grid" id="nl4AdminTop4Grid"></div><p class="note">Admin-only diagnostic. It reads the Admin Model's already-calculated title probabilities and shared live match results. It does not change Elo, evidence weights, match probabilities, Monte Carlo simulations, or Public Model history.</p>`;
  const wrap=table.closest('.nl4-title-probability-table-wrap')||table;
...
function parseProbabilityRows(){
  ...
  const candidates=[
    ...table.querySelectorAll('.nl4-title-probability-row'),
    ...table.querySelectorAll('[data-team],[data-club]'),
  ...
  const rows=unique.map(row=>{
    const clubEl=
      row.querySelector('.nl4-title-team b') ||
      row.querySelector('.nl4-title-team strong') ||
      row.querySelector('[data-team-name]') ||
      row.querySelector('td:first-child strong') ||
      row.querySelector('td:first-child b') ||
      row.querySelector('td:first-child');

    const titleEl=
      row.querySelector('.nl4-title-cell.title') ||
      row.querySelector('[data-stat="title"]') ||
      row.querySelector('[data-title-probability]') ||
      row.querySelector('td:nth-child(2)');

    let club=(
      row.dataset.team ||
      row.dataset.club ||
      clubEl?.dataset?.teamName ||
      clubEl?.textContent ||
      ''
    ).replace(/\s+/g,' ').trim();

    let titleText=(
      titleEl?.dataset?.titleProbability ||
      titleEl?.textContent ||
      ''
    ).trim();

    let m=titleText.match(/([0-9]+(?:\.[0-9]+)?)\s*%/);

    // Final fallback for the div-based model rows.
    if((!club || !m) && row.textContent){
  ...
  const dedup=new Map();
  rows.forEach(r=>{
    if(!dedup.has(r.club) || r.titleProb>dedup.get(r.club).titleProb) dedup.set(r.club,r);
  });
  return [...dedup.values()].sort((a,b)=>b.titleProb-a.titleProb||a.club.localeCompare(b.club));
}
...
async function loadResults(){
  ...
  try{
    ...
    const seen=new Set(); return out.filter(r=>{const k=[r.home,r.away,r.hs,r.as,r.kickoff_at||r.matchday].join('|');if(seen.has(k))return false;seen.add(k);return true;});
  }catch(_){return []}
}
...
async function render(){
  ...
  if(rows.length<4){
    ...
    if(status) status.textContent=`WAITING FOR ADMIN MODEL • ${rows.length}/4 READ`;
    if(summary) summary.textContent='The interpretation panel is connected, but the Admin Model probability table has not rendered at least four readable club rows yet.';
    return;
  }
  ...
  const currentSignature=JSON.stringify(rows.map(x=>[x.club,Number(x.titleProb).toFixed(4)]));
  const previousSignature=prev?.signature||'';
  if(!prev || Number(prev.completedCount)!==completedCount || previousSignature!==currentSignature){
    writeCache({
      completedCount,
...
const timer=setInterval(refreshWhenReady,500);
