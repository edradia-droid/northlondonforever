...
let controls=null;if(renderer){controls=new OrbitControls(camera,renderer.domElement);controls.target.set(0,0,4);controls.enableDamping=true;controls.minDistance=52;controls.maxDistance=155;controls.maxPolarAngle=Math.PI*.48;controls.minPolarAngle=.22;}
...
function line(w,h,x,z){const m=new THREE.Mesh(new THREE.PlaneGeometry(w,h),new THREE.MeshBasicMaterial({color:0xffffff,transparent:true,opacity:.72,side:THREE.DoubleSide}));m.rotation.x=-Math.PI/2;m.position.set(x,.025,z);group.add(m)}
...
function select(name,e){document.querySelectorAll(".player-row").forEach(x=>x.classList.remove("selected"));e.classList.add("selected");const p=players.get(name);if(p&&controls){controls.target.copy(p.position);camera.position.set(p.position.x+34,48,p.position.z+42)}}
fillLists();
function setCamera(mode){if(mode==="top"){camera.position.set(0,125,.1);if(controls) controls.target.set(0,0,0)}else if(mode==="tactical"){camera.position.set(0,70,118);if(controls) controls.target.set(0,0,5)}else{camera.position.set(0,55,92);if(controls) controls.target.set(0,0,8)}}
