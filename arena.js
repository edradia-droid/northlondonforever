...
function select(name,e){document.querySelectorAll(".player-row").forEach(x=>x.classList.remove("selected"));e.classList.add("selected");const p=players.get(name);if(p){camera.position.set(p.position.x+34,48,p.position.z+42)}}
fillLists();
function setCamera(mode){if(mode==="top"){camera.position.set(0,125,.1)}else if(mode==="tactical"){camera.position.set(0,70,118)}else{camera.position.set(0,55,92)}}
