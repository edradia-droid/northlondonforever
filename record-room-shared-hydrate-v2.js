...
(function(){
  ...
  function hasRealLocalData(){
    ...
    return TEAMS.some(team=>{
      if((db[team]?.players||[]).some(p=>['appearances','starts','minutes','goals','assists','yellowCards','redCards','mom','shots','shotsOnTarget','chancesCreated','tackles','interceptions','saves'].some(k=>(Number(p[k])||0)>0))) return true;
      return Object.values(db[team]?.fixtureData||{}).some(s=>s&&(
        (s.homeLineup||[]).some(Boolean)||(s.awayLineup||[]).some(Boolean)||(s.homeSubs||[]).length||(s.awaySubs||[]).length||(s.events||[]).length||norm(s.manOfTheMatch)||
  ...
  async function hydrate(force=false){
    ...
    if(!force&&hasRealLocalData()){status('SHARED SYNC • LOCAL DATA KEPT',true);return {available:true,skipped:true};}
  ...
  const start=()=>setTimeout(()=>hydrate(false),500);
